...
# 建立快速啟動腳本
echo "⚡ 建立快速啟動腳本..."
cat > run_pinecode.sh << 'EOF'
#!/bin/bash
echo "🚀 FlexComponent System - Pinecode Version"
echo "正在啟動..."

# 檢查 Python 依賴
python3 -c "import fastapi, uvicorn, pydantic" 2>/dev/null || {
    echo "📦 安裝依賴..."
    pip install --user fastapi uvicorn pydantic python-multipart
}
...
chmod +x run_pinecode.sh

echo ""
echo "✅ 修復完成！"
echo ""
echo "🎯 現在你可以執行："
echo "1. python backend/main.py     # 直接啟動"
echo "2. ./run_pinecode.sh          # 使用啟動腳本"
echo ""
echo "📊 目錄結構："
ls -la
